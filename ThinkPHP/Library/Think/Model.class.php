...
class Model {
	...
	public function selectPage($options=array()) {
		
        $pk   =  $this->getPk();
        if(is_string($options) || is_numeric($options)) {echo 'aaaaaaaa';
            // 根据主键查询
            if(strpos($options,',')) {
                $where[$pk]     =  array('IN',$options);
            }else{
            ...
            $options            =  array();
            $options['where']   =  $where;
        }elseif (is_array($options) && (count($options) > 0) && is_array($pk)) {echo 'bbbbbb';
            // 根据复合主键查询
            $count = 0;
            foreach (array_keys($options) as $key) {
                if (is_int($key)) $count++; 
        ...
        } elseif(false === $options){echo 'ccccc'; // 用于子查询 不查询只返回SQL
        	$options['fetch_sql'] = true;
        }
        // 分析表达式
        $options    =  $this->_parseOptions($options);//echo  "<meta charset='UTF-8' /><br>".__FILE__.' line:'.__LINE__.'============'."<pre>";print_r($options);exit;
        // 判断查询缓存
        if(isset($options['cache'])){
            $cache  =   $options['cache'];
            $key    =   is_string($cache['key'])?$cache['key']:md5(serialize($options));
        ...
        //$this->count_options = $options;
        
        $recordNum = $this->table($options['table'])->join($options['join'])->where($options['where'])->count();
        
        //$recordNum = $this->count();
        list($page_html, $limit_start, $limit_num) = getpage($recordNum);
        $this->limit($limit_start, $limit_num);
		...
		$tmp = array('page'=>array($page, $pagesize));
		
		$options = array_merge($tmp, $options);
		/**/
		//echo  "<meta charset='UTF-8' /><br>".__FILE__.' line:'.__LINE__.'============'."<pre>";print_r($options);exit;
        ...
        $resultSet = array('data'=>$resultSet, 'page_html'=>$page_html);
        return $resultSet;
    }
    // 查询成功后的回调方法
    ...
    protected function _parseOptions($options=array()) {
        ...
        $this->options  =   array();
        // 表达式过滤
        $this->_options_filter($options);//echo  "<meta charset='UTF-8' /><br>".__FILE__.' line:'.__LINE__.'============'."<pre>";print_r($options);exit;
        return $options;
    }
    // 表达式过滤回调方法
