...
<div class='container mw-800px'>
  ...
  <div id='titlebar'>
    <div class='heading'>
      <span class='prefix' title='USER'><?php if(empty($vo[id])): ?>添加新用户<?php else: ?>编辑用户<?php endif; ?></span>
    </div>
  </div>	
	
	<FORM method="post" class="form-condensed" id="frm" <?php if(empty($vo["id"])): ?>onsubmit="return checkPasswd();"<?php endif; ?> action="/index.php/Users/add">
	
	<table align='center' class='table table-form'>
		<tr>
        <th class='w-90px'>用户名：</th>
        <td class='w-p40'><input type="text" name="retailer_name" value="<?php echo ($vo["retailer_name"]); ?>" /></td><td></td><td></td>
       </tr>
      
      <?php if(empty($vo[id])): ?><tr>
        <th class='w-90px'>登陆密码：</th>
        <td class='w-p40'><input type="password" id="passwd" name="passwd" value="" /></td><td></td><td></td>
       </tr><?php endif; ?> 
       
       
       <tr>
        <th class='w-90px'>角色：</th>
        <td class='w-p40'><select id="" name="role_id" onchange="" ondblclick="" class="form-control" ><option value="" >请选择用户角色</option><?php  foreach($roleNames as $key=>$val) { if(!empty($vo[role_id]) && ($vo[role_id] == $key || in_array($key,$vo[role_id]))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select></td><td></td><td></td>
       </tr>
       
       
       <tr>
        <th class='w-90px'>用户等级：</th>
        <td class='w-p40'><select id="" name="level" onchange="" ondblclick="" class="form-control" ><option value="" >请选择等级</option><?php  foreach($levels as $key=>$val) { if(!empty($vo[level]) && ($vo[level] == $key || in_array($key,$vo[level]))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select></td><td></td><td></td>
       </tr>
       ...
       <tr>
        ...
        <td class='w-p40'><textarea name="memo" cols="50" rows="3"><?php echo ($vo["memo"]); ?></textarea></td><td></td><td></td>
       </tr>
