...
<div class='container mw-800px'>
  ...
  <div id='titlebar'>
    <div class='heading'>
      <span class='prefix' title='USER'><?php if(empty($vo[id])): ?>添加角色<?php else: ?>编辑角色<?php endif; ?></span>
    </div>
  </div>	
	
     <FORM method="post" id="frm"  class="form-condensed"  onsubmit="return checkForm();" action="/index.php/Role/add">
     
	     <?php if(!empty($role["id"])): ?><input type="hidden" name="id" value="<?php echo ($role["id"]); ?>" /><?php endif; ?>
	     
	     <table align='center' class='table table-form'>
	     
	     <tr>
	     	<th >角色名：</th><td><input type="text" name="name" id="name" value="<?php echo ($role["name"]); ?>" /></td>
	     </tr>
	     
	     <tr>
	     	<th >备注：</th><td><textarea name="remark" cols="50" rows="3"><?php echo ($role["remark"]); ?></textarea></td>
	     </tr>
