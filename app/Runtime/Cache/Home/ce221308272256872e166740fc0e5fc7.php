...
<div id="wrap">
 ...
 <div class='container mw-900px'>
  ...
  <div id='titlebar'>
    <div class='heading'>
      <span class='prefix' title='USER'>添加售出记录</span>
    </div>
  </div>	    
               <FORM method="post" action="">
               
               <INPUT type="hidden" name="retailer2goods_id" value="<?php echo ($vo["id"]); ?>">
               <INPUT type="hidden" name="retailer_id" value="<?php echo ($vo["retailer_id"]); ?>">
               
               <table>
               
               <tr>
				       <td>Sku</td> <td><?php echo ($_GET['sku']); ?></td>
			   </tr>	
			 ...
			 <tr>
					   ...
					   <table class="tbl2 table-bordered">
					   <thead>
							<tr>
							<th class="center">品牌</th>
							<th class="center">语言</th>
							<th class="center">颜色</th>
							<th>XS售出数</th><th>S售出数</th><th>M售出数</th><th>L售出数</th><th>XL售出数</th><th>XXL售出数</th>
...
<?php if(is_array($vo["plans_attr"])): $kk = 0; $__LIST__ = $vo["plans_attr"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$attr): $mod = ($kk % 2 );++$kk;?><tr  <?php if($kk == 1): ?>id="rowBase"<?php endif; ?> >
							...
							<td><?php echo ($attr["color"]); ?></td>
								...
								<td>
								 <?php if(($attr["num_xs"]) > $attr["num_xs_sold"]): ?><input type="text" class="w-40px" name="amount_ready[<?php echo ($kk); ?>][xs]" value="" /><br><?php endif; ?>
								 (已<?php echo ($attr["num_xs_sold"]); ?>/共<?php echo (intval($attr["num_xs"])); ?>)
								</td>
								<td>
								 <?php if(($attr["num_s"]) > $attr["num_s_sold"]): ?><input type="text" class="w-40px" name="amount_ready[<?php echo ($kk); ?>][s]" value="" /><br><?php endif; ?>
								(已<?php echo ($attr["num_s_sold"]); ?>/共<?php echo (intval($attr["num_s"])); ?>)
								</td>
								<td>
								 <?php if(($attr["num_m"]) > $attr["num_m_sold"]): ?><input type="text" class="w-40px" name="amount_ready[<?php echo ($kk); ?>][m]" value="" /><br><?php endif; ?>
								(已<?php echo ($attr["num_m_sold"]); ?>/共<?php echo (intval($attr["num_m"])); ?>)
								</td>
								<td>
								 <?php if(($attr["num_l"]) > $attr["num_l_sold"]): ?><input type="text" class="w-40px" name="amount_ready[<?php echo ($kk); ?>][l]" value="" /><br><?php endif; ?>
								(已<?php echo ($attr["num_l_sold"]); ?>/共<?php echo (intval($attr["num_l"])); ?>)
								</td>
								<td>
								 <?php if(($attr["num_xl"]) > $attr["num_xl_sold"]): ?><input type="text" class="w-40px" name="amount_ready[<?php echo ($kk); ?>][xl]" value="" /><br><?php endif; ?>
								(已<?php echo ($attr["num_xl_sold"]); ?>/共<?php echo (intval($attr["num_xl"])); ?>)
								</td>
								<td>
								 <?php if(($attr["num_xxl"]) > $attr["num_xxl_sold"]): ?><input type="text" class="w-40px" name="amount_ready[<?php echo ($kk); ?>][xxl]" value="" /><br><?php endif; ?>
								(已<?php echo ($attr["num_xxl_sold"]); ?>/共<?php echo (intval($attr["num_xxl"])); ?>)
								</td>
