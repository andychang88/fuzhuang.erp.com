...
<meta charset='utf-8'>
               <FORM method="get" id="search_frm"  action="/index.php/Finance/index">
               <table class="search_table">
               
               <tr>
               	<td>分销商</td><td><select id="" name="retailer_id" onchange="" ondblclick="" class="" ><option value="" >请选择分销商</option><?php  foreach($retailers as $key=>$val) { if(!empty($retailer_id_selected) && ($retailer_id_selected == $key || in_array($key,$retailer_id_selected))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select></td>
               	<td>产品单号</td><td><input  class="w-120px" type="text" name="orderno" value="<?php echo ($_GET['orderno']); ?>" /></td>
...
<table class='table table-condensed table-hover table-striped tablesorter table-fixed' >
    ...
    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr  class='text-center' >
	        ...
	        <td><a href="/index.php/Retailer/myGoods?orderno=<?php echo ($vo["orderno"]); ?>" target="_blank"><?php echo ($vo["orderno"]); ?></a></td>
	        <td><?php echo ($vo["totalAmountShouldPaid"]); ?></td>
	        <td><?php echo ($vo["totalAmountAlreadyPaid"]); ?></td>
	        ...
	        <td><?php echo (format_time($vo["paid_time"])); ?></td>
	        ...
	        <?php if($vo["audit_status"] == 0 ): ?><a href="/index.php/Finance/audit/id/<?php echo ($vo["id"]); ?>/status/1">审核通过</a>
	        	<a href="/index.php/Finance/audit/id/<?php echo ($vo["id"]); ?>/status/2">审核驳回</a>
	        	<br>
	        	<a onclick="return confirm('确定要设置为结算完成吗？设置结算完成后分销商就可以录入售出数据了！')" href="/index.php/Finance/clean/id/<?php echo ($vo["id"]); ?>/retailer2goods_id/<?php echo ($vo["retailer2goods_id"]); ?>">结算完成</a><?php endif; ?>
	        
	        
	        <?php if($vo["audit_status"] == 1): ?>审核已通过
    ...
    <tr><td class="pager" colspan=10><div class="viciao"><?php echo ($page); ?></div></td></tr>
    
    
	<?php if(empty($data)): ?><tr><td  class='text-center'  colspan=10>没有数据</td></tr><?php endif; ?>
