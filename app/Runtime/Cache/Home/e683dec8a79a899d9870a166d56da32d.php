...
<style type="text/css">
.bar_icon{
display:inline-block;overflow:hidden;width:15px;height:15px;font-size:0;line-height:0;color:rgba(0,0,0,0);vertical-align:middle;
}
.floatleft{float:left;padding-left:20px;}
.iconTitle{width: 50%;padding-bottom:20px;}
.search_table{margin-left:20px;}
.canvaswidth{width:600px;padding-left:40px}
</style>


	<FORM method="get" id="search_frm"  action="" >
               <table class="search_table">
               
               <tr>
               	<td>统计时间范围：</td><td><input  class="w-120px form-date" type="text" name="starttime" value="<?php echo ($starttime); ?>" /> ~ <input  class="w-120px form-date" type="text" name="endtime" value="<?php echo ($endtime); ?>" /></td>
				
				<td colspan = 2>
               	<button type='submit' id='submit' class='btn btn-submit btn-primary'>提交</button>
               	</td>
               </tr>
    ...
    <h1 class="text-center iconTitle " id="iconTitle"><?php echo ($iconTitle); ?></h1>
   
    <div style="width: 50%" class="floatleft" id="main_data">
			<canvas id="canvas" height="300" width="600"></canvas>
	</div>
	<div class="floatleft"><?php echo ($icon_intro); ?></div>
	

	<script>

	var pieData = [<?php echo ($datasets); ?>];
	var barChartData = {
		labels : [<?php echo ($labels); ?>],
		datasets : [<?php echo ($datasets); ?>]
	}
	
	<?php if(empty($datasets)): ?>$('#main_data').html('<div class="text-center">没有找到待统计数据</div>');
	<?php else: ?>
	window.onload = function(){
		var ctx = document.getElementById("canvas").getContext("2d");
		window.myPie = new Chart(ctx).Pie(pieData);
		$('#iconTitle').width($('#canvas').width()+'px');
	}<?php endif; ?>
	</script>
